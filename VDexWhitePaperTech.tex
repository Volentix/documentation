...
\item to stake and accumulate value by purchasing VTX.
\item to ensure users will partake in order book settlement with their VDex interface in order to decentralize the process.
\end{enumerate}
...
Pre-sale VTX holders can participate in an augmented\cite{21} vote for the justification of token allocations percentages using Venue in a campaign tailored to this purpose. Augmented votes will require voters to take an easy test which proves they acknowledge facts gathered in the Venue campaign concerning the token allocation percentages. Participants to the elaboration of this test grid will compile facts linked to publications gathered regarding this issue and will earn VTX in the process.
The resulting vote is therefore 'augmented' by ensuring all voters
vote with the same knowledge regarding this issue.
